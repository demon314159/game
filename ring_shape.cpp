...
RingShape::RingShape(float radius_o, float radius_i, float height)
    : m_radius_o(radius_o)
    , m_radius_i(radius_i)
    , m_height(height)
    , m_count_mode(true)
...
void RingShape::define_shape()
{
    ...
    for (int j = 0; j < steps; j++) {
        ...
        float xa0 = m_radius_o * cosa;
        float xb0 = m_radius_o * cosb;
        float za0 = m_radius_o * sina;
        float zb0 = m_radius_o * sinb;
        float xa1 = m_radius_i * cosa;
        float xb1 = m_radius_i * cosb;
        float za1 = m_radius_i * sina;
        float zb1 = m_radius_i * sinb;
        add_face({xa0, -dy, za0}, {xb0, -dy, zb0}, {xb0, dy, zb0}, {xa0, dy, za0}, true);
        add_face({xa1, -dy, za1}, {xb1, -dy, zb1}, {xb1, dy, zb1}, {xa1, dy, za1}, false);
        add_face({xa0, -dy, za0}, {xa1, -dy, za1}, {xb1, -dy, zb1}, {xb0, -dy, zb0}, true);
        add_face({xa0, dy, za0}, {xa1, dy, za1}, {xb1, dy, zb1}, {xb0, dy, zb0}, false);
    }
}
